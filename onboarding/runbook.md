...
| 单 DOI snapshot/quality 修复 | `check-snapshot`、`snapshot_expected.py`、`repair-markdown-quality` | 某个 DOI 的 `expected.json`、`extracted.md`、fresh Markdown review 或 `markdown-quality.json` 缺失、过期或 fail |
...
先运行 check-snapshot 或 repair-markdown-quality；链路会通过 PROVIDER_ONBOARDING_AGENT_CLI 重新读取当前 extracted.md 做 fresh review，不再只信旧 markdown-quality.json。
