...
10. 将最终人工语义审查写入 `onboarding/reviews/<NAME>.yml`，每个 non-null fixture 和 `extra_fixtures` 都要有 `baseline_markdown_path`、`baseline_markdown_sha256`、`review_notes`、`sample_representative: true`、`markdown_semantic_reviewed: true`、`issues`、`assertions`、`fixes`；`fixes[].issue_ids` 必须引用已有 issue，`fixes[].test_names` 至少列出一个 provider-local 测试。
