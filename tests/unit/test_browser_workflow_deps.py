...
from paper_fetch.providers import _cloakbrowser, _pdf_fallback
...
def test_default_browser_workflow_deps_match_production_functions() -> None:
    ...
    assert deps.load_runtime_config is _cloakbrowser.load_runtime_config
    assert deps.ensure_runtime_ready is _cloakbrowser.ensure_runtime_ready
    assert deps.probe_runtime_status is _cloakbrowser.probe_runtime_status
    assert deps.fetch_html_with_flaresolverr is _cloakbrowser.fetch_html_with_cloakbrowser
    assert deps.warm_browser_context_with_flaresolverr is _cloakbrowser.warm_browser_context_with_cloakbrowser
    ...
    assert deps.pdf_browser_context_seed is _cloakbrowser.warm_browser_context_with_cloakbrowser
    assert deps.refresh_browser_context_seed is _cloakbrowser.warm_browser_context_with_cloakbrowser
