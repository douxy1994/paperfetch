...
- 如果目标论文属于 `wiley`、`science`、`pnas`、`ams`、`acs`、`iop`、`mdpi` 这类依赖浏览器运行时的 provider，在第一次抓取前用 `provider_status()` 确认 CloakBrowser runtime 健康后再抓取。
- 如果 `wiley`、`science`、`pnas`、`ams`、`acs`、`iop`、`mdpi` 这类依赖浏览器运行时的抓取首次失败，可以在排除明显配置错误后最多再重试 `2` 次；重试时优先绕过缓存，并确认本地 CloakBrowser runtime 健康状态。若仍失败，要明确告诉用户失败发生在浏览器链路。
...
10. 如果目标 provider 是 `wiley`、`science`、`pnas`、`ams`、`acs`、`iop` 或 `mdpi`，在第一次抓取前调用 `provider_status()` 确认本地 CloakBrowser runtime 健康。
11. 如果提供方凭证、Wiley / Science / PNAS / AMS / ACS / IOP / MDPI 的本地运行时状态，或 IEEE Xplore 访问上下文可能影响结果，在第一次抓取前调用 `provider_status()`。
...
- 当你需要提供方凭证、下载目录行为，Wiley / Science / PNAS / AMS / ACS / IOP / MDPI 运行时要求，或 IEEE 访问边界时，读取 [`references/environment.md`](references/environment.md)。
