...
离线 release asset 包含 4 个 Linux ABI 自解压 `.sh` 安装器和 1 个 Windows x86_64 安装器。macOS tarball 使用同一构建脚本在目标 macOS 架构上生成，并由 macOS CI 验证安装、headful preset 和 CloakBrowser 浏览器启动 smoke，但当前不作为 release asset 发布。
...
macOS 离线安装包使用同一构建脚本在目标 macOS 架构上生成 `.tar.gz` bundle；CI 会验证 macOS 安装路径，但当前 release asset 仍只发布 Linux x86_64 `.sh` 和 Windows x86_64 安装器。macOS 可见浏览器调试使用：
