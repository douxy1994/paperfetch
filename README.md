...
离线包分为 Linux / macOS ABI tarball 和 Windows x86_64 安装器。当前自动发布的 release asset 包含 Linux x86_64 tarball 和 Windows x86_64 安装器；macOS tarball 使用同一构建脚本在目标 macOS 架构上生成。
...
安装器会注册 CloakBrowser 默认 headless 环境，并在 `offline.env` 默认启用普通 Chrome browser UA，降低 AGU/Wiley 页面进入 Cloudflare challenge 的概率。受限环境可在 `offline.env` 中设置 `CLOAKBROWSER_BINARY_PATH` 指向预装浏览器；如果 GUI/WSLg 环境下仍被 challenge，再把 `CLOAKBROWSER_HEADLESS=false`，或在 Linux tarball 中使用 `--preset=wslg`。
...
Linux / macOS tarball 是预安装 runtime 包，不再是源码快照：解压后主要包含 `bin/` 启动器、`runtime/site-packages/` 已安装 Python 包、`formula-tools/` 和 skill 文件。

Ubuntu 24.04 系统默认 Python 版本 3.12，Ubuntu 26.04 为 3.14。

解压后执行安装脚本。默认安装到 `~/.local/share/paper-fetch-skill`，也可以用 `--install-dir <path>` 指定固定目录：
...
Linux / macOS 离线安装会优先把 `MATHML_TO_LATEX_NODE_BIN` 指向包内 Playwright Node，避免依赖系统 PATH 上的 `node`。

macOS 需要可见浏览器窗口调试时改用：
...
Linux / macOS 下载与目标机平台、架构和 Python 版本匹配的新版 tarball，解压后运行 `install-offline.sh`。默认安装目录固定为 `~/.local/share/paper-fetch-skill`，升级时会清理旧 runtime payload、移除旧源码/构建残留，并保留安装目录内的 `offline.env`。若希望复用外部 env 文件且不修改它，使用 `--reuse-env-file`：
