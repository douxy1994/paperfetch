...
离线 release asset 包含 4 个 Linux ABI 自解压 `.sh` 安装器、1 个 macOS CPython 3.12 tarball 和 1 个 Windows x86_64 安装器。macOS tarball 使用同一构建脚本在 `macos-latest` 上按 runner 架构生成，并由 macOS CI 验证安装、headful preset 和 CloakBrowser 浏览器启动 smoke。
...
macOS 离线 release asset 是 CPython 3.12 tarball；下载与目标机架构匹配的 `paper-fetch-skill-offline-macos-*-cp312.tar.gz` 后解压运行。macOS 可见浏览器调试使用：
