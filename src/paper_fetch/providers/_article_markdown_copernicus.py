"""Copernicus NLM/JATS XML extraction helpers."""
...
def extract_copernicus_authors(root: ET.Element) -> list[str]:
    authors: list[str] = []
...
def extract_copernicus_metadata(
    root: ET.Element,
    *,
    ...
    metadata.update(
        {
            ...
            "authors": extract_copernicus_authors(root) or list(base.get("authors") or []),
...
def _render_paragraph_texts(parent: ET.Element | None) -> list[str]:
    ...
    for child in iter_children(parent):
        ...
        if local_name == "title":
            continue
        if local_name == "p":
            text = render_inline_text(child, skip_local_names=COPERNICUS_BLOCK_LOCAL_NAMES)
            if text:
                texts.append(text)
            continue
...
def _supplementary_entries(root: ET.Element, source_url: str) -> list[dict[str, Any]]:
    ...
    for node in root.iter():
        ...
        title = normalize_text(str(node.get(XLINK_TITLE) or node.get("content-type") or "")) or text or "Supplementary material"
...
def extract_copernicus_references(root: ET.Element) -> list[dict[str, Any]]:
    references: list[dict[str, Any]] = []
...
def parse_copernicus_xml(
    ...
    *,
    ...
    metadata = extract_copernicus_metadata(root, base_metadata=base_metadata, source_url=source_url)
    article_meta = _article_meta(root)
    abstract_node = first_child(article_meta, "abstract")
    abstract_text = normalize_text("\n\n".join(_render_paragraph_texts(abstract_node)))
    abstract_sections = (
    ...
    for entry in supplement_entries:
        if not any(normalize_text(str(item.get("key") or "")) == normalize_text(str(entry.get("key") or "")) for item in assets):
            assets.append(entry)
    markdown_text = normalize_lines([*body_lines, *back_lines])
    references = extract_copernicus_references(root)
    if references:
...
def build_copernicus_markdown_document(
    ...
    *,
    ...
    lines.append("- Provider: `copernicus`")
