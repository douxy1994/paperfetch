...
def server_instructions() -> str:
    return (
        ...
        "`preferred_providers` may include `elsevier`, `springer`, `wiley`, `science`, `pnas`, `ams`, `acs`, `mdpi`, `ieee`, `arxiv`, `copernicus`, or `crossref`. "
        ...
        "`pnas`, `ams`, `acs`, and `mdpi` require the local browser runtime but no legacy local "
        "rate-limit env vars; AMS publishes `ams_html` or `ams_pdf` and ignores `citation_xml_url`; ACS publishes `acs`; MDPI publishes `mdpi_html` or `mdpi_pdf` and does not use an XML route. `ieee` uses "
        ...
        "Inline ImageContent still only comes from body figures. Wiley/Science/PNAS/AMS/ACS/MDPI support "
...
def fetch_tool_description() -> str:
    return (
        ...
        "`wiley_browser` on success. `science`, `pnas`, `ams`, `acs`, and `mdpi` routes use "
        "provider-managed browser runtime HTML plus seeded-browser publisher PDF/ePDF repo-local "
        "workflows; AMS publishes `ams_html` or `ams_pdf` and does not request `citation_xml_url` / `/doc/...xml`; ACS publishes `acs`; MDPI publishes `mdpi_html` or `mdpi_pdf` and does not use an XML route. `ieee` uses landing metadata, "
        ...
        "Wiley/Science/PNAS/AMS/ACS/MDPI support body/all assets on successful CloakBrowser HTML routes while keeping "
