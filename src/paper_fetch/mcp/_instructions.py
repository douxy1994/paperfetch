...
def server_instructions() -> str:
    return (
        ...
        "`preferred_providers` may include `elsevier`, `springer`, `wiley`, `science`, `pnas`, `ieee`, `copernicus`, or `crossref`. "
        ...
        "publishing `ieee_html` or `ieee_pdf` when those routes return usable full text. `copernicus` uses "
        ...
        "their PDF/ePDF fallback routes remain text-only. Springer, IEEE, and Copernicus PDF fallback "
...
def fetch_tool_description() -> str:
    return (
        ...
        "`ieee_html` or `ieee_pdf`. `copernicus` uses direct HTTP landing discovery, public NLM/JATS XML, "
        ...
        "PDF/ePDF fallback text-only, and Springer/IEEE/Copernicus PDF fallback is also text-only "
        "in this version. Set "
