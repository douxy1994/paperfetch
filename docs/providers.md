...
| `ams` | 依赖 Crossref | `DOI landing -> CDP browser HTML -> CDP browser-seeded publisher PDF` | HTML 路线支持 `none` / `body` / `all`；PDF fallback 在 `body/all` 且允许落盘时提取 PDF 图片到 `body_assets/` | 中 | 需要本地 browser runtime；`CLOAKBROWSER_CDP_ENDPOINT` 可显式复用外部浏览器，未设置时自动启动 cloakbrowser Chrome；`PAPER_FETCH_AMS_STORAGE_STATE_JSON` 仅为 legacy runtime override；AMS 只使用 `journals.ametsoc.org/view/...xml` landing HTML 和 PDF fallback；显式忽略 `citation_xml_url`，不请求 `/doc/...xml`，不暴露 XML/JATS source；HTML 成功公开 `ams_html`，PDF fallback 公开 `ams_pdf` |
...
```text
resolve DOI / landing URL
-> legacy /articles/{doi}/full 或 DOI resolver 跟随到 canonical /journals/{journal}/articles/{doi}/full
...
- Frontiers canonical XML 路径需要 journal slug，例如 `/journals/marine-science/articles/10.3389/fmars.2023.1101972/xml`；旧式 `/articles/{doi}/xml` 会返回 404，因此 provider 先通过 landing redirect、landing metadata 或 canonical URL 发现 `/journals/{journal}/articles/{doi}` 路由，再请求 `/xml` 和 `/pdf`。
...
- `springer`、`wiley`、`science`、`pnas`、`ieee`、`copernicus`、`ams`、`mdpi`、`royalsocietypublishing`、`annualreviews`、`plos`、`frontiers`、`oxfordacademic`、`acs`、`iop`、`aip` 在 `probe_official_provider()` 和 `has_fulltext()` 中都只依赖 Crossref / landing-page / DOI 信号，不再调用 publisher metadata API。
...
- 当前不会下载 arXiv TeX 源码做本地 TeX / LaTeX 全文转换；全文仍只消费 arXiv official HTML。source 包只用于 official HTML 成功但正文图片是缺失占位符时恢复 figure 资产；若 official HTML 缺失或质量不过关，即使 TeX 源码可能存在，也会直接进入 PDF fallback。
