...
Discovery worker 只能写 `output_manifest`。它不能写代码、fixture、tests、shared docs，也不能 commit。
...
Discovery worker 必须写一个 `ProviderManifest` YAML 到 `output_manifest`，并且必须通过 `docs/ai-onboarding/provider-manifest.schema.json`。
