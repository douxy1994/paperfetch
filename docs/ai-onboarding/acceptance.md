...
- `docs/ai-onboarding/access-reviews/<provider>.yml` exists, passes `docs/ai-onboarding/access-review.schema.json`, has `status: approved`, and has `may_continue: true`.
...
- Full local orchestration may use `python3 scripts/onboard_from_manifests.py run --manifest docs/ai-onboarding/manifests/<provider>.yml --until merge-ready`; worker dispatch must go only through `PROVIDER_ONBOARDING_AGENT_CLI`.

## Markdown Review Gates

- `docs/ai-onboarding/reviews/<provider>.yml` exists and passes `docs/ai-onboarding/provider-review.schema.json`.
...
- Worker completion summary is secondary; acceptance uses `docs/ai-onboarding/reviews/<provider>.yml` as the durable source.
...
- `docs/ai-onboarding/automation-roadmap.md` documents which onboarding steps are automatable and which remain operator-controlled.
