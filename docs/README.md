...
- [`deployment.md`](deployment.md)
  ...
  - 讲 Wiley / Science / PNAS / AMS / ACS / MDPI 的 repo-local 浏览器工作流、本地 `scripts/dev-preflight.sh` 门禁和 CI 测试耗时信号。
...
- 常见值包括 `full_size`、`preview`。旧的通用 HTTP-first 路径仍可能保留 `playwright_canvas_fallback` 诊断，但 `wiley` / `science` / `pnas` / `ams` / `acs` 的 HTML 资产主链路不再输出这个 tier。
...
- `wiley` / `science` / `pnas` / `ams` / `acs` / `mdpi` 的 challenge 恢复链路只接受能识别为图片的 CloakBrowser image payload，包括浏览器导出的 PNG 和原始 SVG；不会再把图片文档 screenshot 裁剪成正文图片资产，也不会把 challenge HTML 保存成图片。
...
- live runner supported provider 包含 `elsevier`、`springer`、`wiley`、`science`、`pnas`、`ieee`、`arxiv`、`ams`、`acs`、`mdpi`、`copernicus`、`plos`；arXiv、MDPI、Copernicus 与 PLOS 路径和资产语义见 [`providers.md`](providers.md)。
