...
- 安装 Python 依赖、CloakBrowser 运行时依赖和外部公式后端；默认 provider-owned HTML bootstrap 使用 CDP browser workflow
...
- `PAPER_FETCH_AMS_STORAGE_STATE_JSON` 是 legacy runtime override；`paper-fetch auth ams` 不再生成或写入该变量，默认复用 `publisher-browser-profiles/ams/storage-state.json`
