...
离线发布支持 Linux x86_64 和 Windows x86_64。Linux 继续按 CPython ABI 提供 3.11、3.12、3.13、3.14 自解压 `.sh` 安装器，内部 payload 是预安装 runtime 包，不再复制仓库源码快照；Windows 提供一个内置 CPython 3.13 x64 的 Inno Setup 安装器。macOS x86_64 / arm64 tarball 使用同一构建脚本在目标 macOS 架构上生成，并由 CI 验证安装路径；当前 release asset 集合仍只包含 Linux 和 Windows：
...
- `offline-macos-install` 会在 `macos-latest` 上构建本机架构 macOS tarball，执行安装器验证、headful preset 安装布局检查，并用安装后的 `paper-fetch` / `python` 通过 CloakBrowser 启动本机浏览器打开本地 `data:` 页面，确认 macOS 包安装后可实际使用浏览器路径。
...
macOS 目标机使用本机架构 tarball，解压后运行包内安装脚本：
