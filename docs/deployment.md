...
离线 shell tarball 支持 Linux x86_64 和 macOS x86_64 / arm64。当前自动发布的 release asset 包含 Linux x86_64 tarball 和 Windows x86_64 安装器；macOS tarball 使用同一构建脚本在目标 macOS 架构上生成。Linux / macOS tarball 按 CPython ABI 提供 3.11、3.12、3.13、3.14，且内容是预安装 runtime 包，不再复制仓库源码快照；Windows 提供一个内置 CPython 3.13 x64 的 Inno Setup 安装器：
...
paper-fetch-skill-offline-macos-x86_64-cp311.tar.gz
paper-fetch-skill-offline-macos-x86_64-cp312.tar.gz
paper-fetch-skill-offline-macos-x86_64-cp313.tar.gz
paper-fetch-skill-offline-macos-x86_64-cp314.tar.gz
...
- release job 会下载本次运行产出的 `paper-fetch-skill-*` artifacts，确认 Linux x86_64 的 4 个 tarball 和 Windows x86_64 安装器都存在且没有额外文件，然后把它们作为 release assets 上传。
...
Linux / macOS 目标机解压与平台、架构和 Python ABI 匹配的 tarball 后运行安装脚本。默认安装到 `~/.local/share/paper-fetch-skill`：

```bash
tar -xzf paper-fetch-skill-offline-linux-x86_64-cp312.tar.gz
cd paper-fetch-skill-offline-linux-x86_64-cp312
./install-offline.sh --preset=headless --no-user-config
source ~/.local/share/paper-fetch-skill/activate-offline.sh
```

Linux WSLg 或桌面显示环境可用：
...
```bash
./install-offline.sh --install-dir "$HOME/tools/paper-fetch-skill" --preset=headless --no-user-config
source "$HOME/tools/paper-fetch-skill/activate-offline.sh"
```

macOS 需要可见浏览器窗口调试时使用：
...
- `wslg` 仅面向 Linux WSLg 环境。

Shell rc 写入策略：

- Linux / macOS 安装脚本会把 tarball payload 复制到固定安装目录，使用该目录下的 `bin/` 启动器和 `runtime/site-packages/` 已安装 Python 包，复制 Codex / Claude Code / Gemini CLI skill，并注册 MCP。
...
- Linux / macOS：下载与目标机平台、架构和 CPython ABI 匹配的新 tarball，解压后运行 `install-offline.sh`。默认安装目录固定为 `~/.local/share/paper-fetch-skill`，升级时会备份安装目录内的 `offline.env`，清理旧 runtime payload 和旧源码/构建残留，把新版 runtime-only payload 复制进去，再写回 `offline.env` 并刷新 shell / skill / MCP managed block。若希望更新时不改动外部 `offline.env`，用 `--reuse-env-file` 指向现有文件；安装脚本不会写入该文件，只会把 shell 启动文件和 Codex fallback config 中的 managed block 替换为新安装目录的 PATH / MCP runtime 路径。

```bash
./install-offline.sh --preset=headless --no-user-config
./install-offline.sh --preset=headless --no-user-config --reuse-env-file /path/to/shared/offline.env
source ~/.local/share/paper-fetch-skill/activate-offline.sh
```
...
- Linux / macOS 安装时会把通过 `PAPER_FETCH_OFFLINE_PYTHON_BIN` / `python3` 选中的解释器路径写入 `runtime/python-bin`，后续 `bin/python`、CLI wrapper 和 MCP 都复用该解释器
- Windows 安装器固定使用包内 CPython 3.13 x64 embeddable runtime；目标机不需要预装 Python
- Linux / macOS 构建阶段用临时 wheelhouse 把项目和依赖安装进 `runtime/site-packages`，然后只打包安装后的 runtime、`bin/` 启动器、公式工具和 skill；目标机安装阶段不运行 pip，不包含源码树、`dist/` 或 `wheelhouse/`
- CloakBrowser Python 包随 Linux / macOS `runtime/site-packages` 和 Windows embedded runtime 分发；浏览器 binary 不随包分发，受限环境可预先安装并设置 `CLOAKBROWSER_BINARY_PATH`，该路径会在 runtime config / provider status 中校验为可执行文件
- Linux / macOS tarball 不包含仓库源码快照和 `tests/` 目录；离线安装目标是运行已打包工具，不在目标机执行项目测试
- Linux / macOS 公式工具使用包内 `formula-tools/bin/texmath`，Windows 使用 `formula-tools/bin/texmath.exe`；目标机不编译 texmath，也不运行 `npm install`
- Linux / macOS 默认写固定安装目录内的 `offline.env`、生成 `activate-offline.sh`、复制 `~/.codex/skills/paper-fetch-skill`、`~/.claude/skills/paper-fetch-skill` 和 `~/.gemini/skills/paper-fetch-skill`，并把离线 CLI PATH、formula tools PATH、`PAPER_FETCH_ENV_FILE`、`PAPER_FETCH_FORMULA_TOOLS_DIR`、`CLOAKBROWSER_HEADLESS` 写入当前 shell 对应启动文件；`offline.env` 的 managed block 默认启用普通 Chrome `PAPER_FETCH_BROWSER_USER_AGENT`，只有显式传 `--user-config` 才会把受标记管理的运行时块合并到 `~/.config/paper-fetch/.env`
...
验证脚本会先确认 tarball 包含 `runtime/site-packages` 和 `bin/` 启动器，且不包含源码树或 build wheelhouse；再用 guard 拦截 `curl`、`git`、`npm`、`playwright` 等命令来确认安装器没有在线下载或目标机 patch 动作，并使用临时 HOME 和 fake `codex` / `claude` / `gemini` CLI 验证 Linux / macOS shell 写入、skill 复制和 MCP remove/add 注册；随后检查 `paper-fetch --help`、`texmath --help`、`cloakbrowser` import、`paper_fetch.mcp.fetch_tool.provider_status_payload`，最后执行 `install-offline.sh --uninstall` 验证用户级集成可清理且不删除安装目录内 `offline.env` 或 runtime，并执行 `--purge` 验证固定安装目录可显式删除。
