...
- Front matter 的 publication watermark 只匹配短 masthead 标签：必须是短文本、无句子标点、token 数受限，并且呈标题式或全大写；`science` / `pnas` / `ams` / `bams` / `acs` 等 provider 词面只作为 provider-scoped keyword 参与判断，长正文句子里出现这些词不能被当作 front matter。
...
- 边界说明：
  - 这条规则目前适用于 `wiley`、`science`、`pnas`、`ams`、`acs` 的 browser workflow HTML 成功路径。
