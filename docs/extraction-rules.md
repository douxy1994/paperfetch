...
- 边界说明：
  ...
  - 本规则只定义 render-state 去重；本地链接改写和下载诊断字段分别见 [已下载的正文图片和公式图片要改写成正文附近的本地链接](#rule-rewrite-inline-figure-links) 与 [下载资产必须保留诊断字段](#rule-asset-download-diagnostic-fields)。
...
- 这条规则约束的是：HTML-derived Markdown 中已经识别出的数字引用 sentinel 要稳定渲染为 `<sup>...</sup>`；引用前缀和周边标点要清理到可读形态；同时不能把普通数字文本、年份范围或 Markdown 图片 opener `![...]` 当成引用标点处理。
- 如果违反，用户会看到：数字引用丢失或残留内部 sentinel，`ref. <sup>21</sup>` 这类前缀清理失败，句末出现多余空格/标点，或者正文图片前的空行被标点清理吃掉，形成 `sentence.![Figure]` 这类坏 Markdown。
