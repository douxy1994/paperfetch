...
- Browser workflow 的并发资产下载 fetcher 必须复用 `RuntimeContext` 的 keyed browser manager，并为每个 worker 线程创建私有 context/page；不得为同一 provider profile 的每个资产 fetcher 再启动独立 Chrome，也不得跨线程复用同步 Playwright page/context。
- 并发资产 worker 中创建的 thread-local page/context/manager 必须在同一个 worker 线程内关闭；不能在主线程统一关闭 worker 线程创建的 sync browser 对象，否则容易残留浏览器子进程。
